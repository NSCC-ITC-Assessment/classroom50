...
| File | Where it goes |
|---|---|
| `autograder.py` | `.github/scripts/autograder.py` in your config repo (replaces the diagnostic-stub default that ships with `gh teacher init`). One file grades every assignment in the org. |
...
```sh
# 1. Replace the org-level default autograder in your config repo
cp examples/autograders/cs50/autograder.py \
   <path-to-classroom50>/.github/scripts/autograder.py
# Commit + push to publish via GitHub Pages (~30s for the first deploy).
...
On every submission, the autograde-runner workflow fetches `runner.py` (the runner-side bootstrap), which then fetches this `autograder.py` from your Pages site (or picks up a per-assignment override if the bundle contains one). Inside the `cs50/cli` container:
...
- **Year rollover**: edit `CHECK50_SPEC_PREFIX` at the top of `autograder.py` when CS50 advances `cs50/problems/<year>` (e.g. `2026` → `2027`).
- **Per-assignment exceptions**: drop a per-assignment override at `<classroom>/autograders/<slug>/autograder.py` for any assignment that needs grading logic outside the check50 model (e.g. a project, a non-CS50 problem set). The runner picks per-assignment overrides over this org-level default automatically.
