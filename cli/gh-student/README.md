...
Submit is intentionally minimal: `submitCmd` refreshes the template repo's `.gitignore` and `.github/` (both optional, fetched from the template ref recorded in `.classroom50.yaml`), then commits + pushes to `main`. The autograde shim itself is set once at accept time and never refreshed — actual grading logic lives at the org level (the runner-side bootstrap `runner.py` and the org-level default autograder `autograder.py`, both fetched from the teacher's Pages site at workflow runtime) plus optional per-assignment overrides under `<classroom>/autograders/<slug>/autograder.py`. The runner workflow auto-tags the pushed commit and publishes a release at the submit tag.
