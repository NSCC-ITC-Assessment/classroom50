...
// the org-level default at `.github/scripts/autograder.py` (used
// when no per-assignment override exists). The runner-side
// bootstrap at `.github/scripts/runner.py` downloads the bundle,
// resolves the entrypoint, and execs it. See the Autograders wiki
...
// keyword. `Credentials.Password` typically holds a
// `${{ secrets.NAME }}` reference; the runner re-emits the value
// verbatim into the workflow JSON so secret expansion happens at
// job-load time.
//
