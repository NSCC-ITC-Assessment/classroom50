#!/usr/bin/env python3
"""classroom50 default autograder (diagnostic stub).

Runs whenever an assignment has no per-assignment override at
<classroom>/autograders/<slug>/autograder.py. Echoes every env var
the runner exposed (so teachers can confirm metadata wires up
correctly), writes a vacuous-pass result.json + release-body.md,
and exits 0.

This file lives at .github/scripts/autograder.py in your config
repo. Replace it to grade every assignment in the org with one
autograder (CS50 + check50 is the canonical example — slug becomes
the check50 spec leaf). Or leave this default and write
per-assignment autograders under <classroom>/autograders/<slug>/
autograder.py — the runner picks those over this default.
...
print("=== classroom50 default autograder (diagnostic stub) ===")
