...
reusable workflow on every submission. Responsibilities:
  ...
  4. Resolve the entrypoint:
       ...
       org-level default at .github/scripts/autograder.py
       (fetched from the Pages site root)
...
The runner always exits 0; failures (bundle fetch error, malformed
result.json, autograder rc != 0, etc.) are reported via a synthetic
error result + status=error, so the workflow's release/commit-status
steps still fire and the gradebook still ingests the submission.
...
# means "no per-assignment override"; for the default-autograder
# URL it means publish-pages.yaml hasn't run yet.
FETCH_ATTEMPTS = 3

# Hard cap on the bundle / default-autograder fetches. Bundles fitting
# in 10 MB cover all realistic test suites; the default autograder is
# small but the same ceiling avoids a hostile asset.
...
def default_autograder_url(pages_base_url: str) -> str:
    """The Pages URL for the org-level default autograder.py."""
    return f"{pages_base_url}/{ENTRYPOINT_FILENAME}"
...
def extract_tarball(data: bytes, dest: pathlib.Path) -> None:
    ...
    other unsafe member types — defensive even though the bundle
    comes from the teacher's config repo.
    """
...
def main() -> int:
    ...
    # through to the org-level default).
    ...
    # 2) Resolve the entrypoint: per-assignment > org-level default.
    ...
    if per_assignment.is_file():
        ...
        durl = default_autograder_url(pages_base_url)
        print(
            f"runner: no per-assignment {ENTRYPOINT_FILENAME}; "
            f"fetching default from {durl}"
        )
        try:
            content = fetch_url(durl)
        except (urllib.error.HTTPError, urllib.error.URLError, ValueError) as exc:
            return finalize.error(f"default {ENTRYPOINT_FILENAME} fetch failed: {exc}")
        if content is None:
            return finalize.error(
                f"no {ENTRYPOINT_FILENAME}: per-assignment override missing AND "
                "default not published — confirm publish-pages.yaml ran"
            )
        entrypoint = runtime_dir / ENTRYPOINT_FILENAME
        entrypoint.write_bytes(content)
        print(f"runner: using org-level default entrypoint {entrypoint}")
