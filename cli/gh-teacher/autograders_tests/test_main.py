...
the orchestration glue — the 14 error/synthesis branches that only
fire in composition: bundle fetch, extraction, entrypoint resolution
(per-assignment vs org-level default), subprocess exec, result.json
validation, and Finalizer-driven error synthesis.
...
class TestEntrypointResolution:
    ...
    does the runner exec the per-assignment override or the org-level
    default? Bundle contents drive the choice."""
    ...
    def test_bundle_without_autograder_falls_back_to_default(self, harness):
        ...
        # Both URLs fetched: bundle then default.
        assert any(".tar.gz" in u for u in harness["fetched_urls"])
        assert any(u.endswith("/autograder.py") for u in harness["fetched_urls"])

    def test_bundle_404_falls_back_to_default(self, harness):
        # No per-assignment bundle at all (Pages 404) — runner uses the
        # default autograder. Same outcome as a bundle-without-autograder
    ...
    def test_no_bundle_and_no_default_emits_error_result(self, harness):
        # Worst case: no per-assignment bundle AND no default published.
        # Synthesize an error result so the gradebook still ingests it.
        rc = harness["run"](bundle_response=None, default_response=None)
        assert rc == 0  # Always 0 on the error path; status=error
        result, body, output = _read_outputs(harness)
        assert result["schema"] == "classroom50/result/v1"
        assert result["tests"] == []
        assert "no autograder.py" in body or "no autograder.py" in output
