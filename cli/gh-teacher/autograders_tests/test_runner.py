...
bundle, (2) resolve the entrypoint (per-assignment override or
org-level default), (3) exec it with the right env / cwd, and (4)
validate / synthesize the autograder's outputs. These tests cover
the data-shape invariants the rest of the loop depends on plus the
URL helpers and HTTP retry logic.

Integration testing of the org-level default autograder.py lives in
test_default_autograder.py (subprocess-driven).
"""
...
class TestUrlConstruction:
    ...
    def test_default_autograder_url(self):
        # The org-level default lives at the Pages site root —
        # one fetch per submission when no per-assignment override
        # is in the bundle.
        assert ag.default_autograder_url(
            "https://cs50.github.io/classroom50"
        ) == "https://cs50.github.io/classroom50/autograder.py"
...
class TestDeriveStatusAndSummary:
    ...
    def test_empty_tests_yields_success_with_no_autograder_summary(self):
        # Vacuous pass — the org-level default's "no autograder
        # configured" path. The default explicitly writes status=
        # itself; this fallback covers the case where a per-assignment
        # autograder produces an empty tests array.
