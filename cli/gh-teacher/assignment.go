...
func assignmentAddCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Register an assignment — its template repo and the autograder it\n" +
			...
			"<classroom>/autograders/<slug>/ in the config repo, OR\n" +
			"replace the org-level default at\n" +
			".github/scripts/autograder.py to grade every assignment in\n" +
			"the org with one autograder. See the Autograders wiki page\n" +
			"for the result.json contract and templates (pytest, check50,\n" +
			"custom).",
